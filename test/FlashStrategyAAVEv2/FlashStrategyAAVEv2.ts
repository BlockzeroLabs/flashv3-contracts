...
describe("FlashStrategyAAVEv2 Tests", function () {
  ...
  before(async function () {
    ...
    this.flashStrategyAAVEv2Artifact = <FlashStrategyAAVEv2>await deployContract(this.signers.admin, flashStrategyAAVEv2Artifact, [
      "0xE0fBa4Fc209b4948668006B2bE61711b7f465bAe", principalTokenAddress, "0xdCf0aF9e59C002FA3AA091a46196b37530FD48a8", flashProtocolAddress
    ]);
    ...
    const flashFTokenContract = <FlashFToken>await deployContract(this.signers.admin, flashFTokenArtifact, ['fDAI', 'fDAI']);
    ...
    await expect(this.flashStrategyAAVEv2Artifact.connect(this.signers[1]).setFTokenAddress(flashFTokenContract.address)).to.be.revertedWith("FTOKEN ADDRESS ALREADY SET");

    // Approve the Flash Strategy contract to spend fTokens from user wallet
    await flashFTokenContract.approve(this.flashStrategyAAVEv2Artifact.address, BigNumber.from(2).pow(BigNumber.from(256)).sub(BigNumber.from(1)))
  });
  ...
  it("[account 0] ensure depositing principal fails with error NOT FLASH PROTOCOL", async function () {
    await expect(this.flashStrategyAAVEv2Artifact.depositPrincipal(BigNumber.from(10000).mul(multiplier))).to.be.revertedWith("NOT FLASH PROTOCOL");
  });
  ...
  it("[account 1] should deposit 10,000 DAI principal", async function () {
    ...
    expect(await daiContract.allowance(this.signers.admin.address, this.flashStrategyAAVEv2Artifact.address)).eq(_amount);
  ...
  it("[account 0] should revert when withdrawing principal with error NOT FLASH PROTOCOL", async function () {
    await expect(this.flashStrategyAAVEv2Artifact.withdrawPrincipal(BigNumber.from(10000).mul(multiplier))).to.be.revertedWith("NOT FLASH PROTOCOL");
  });

  it("[account 1] should withdraw 10,000 DAI (principal) and still have yield in contract", async function () {

    // Withdraw the DAI
    await this.flashStrategyAAVEv2Artifact.connect(this.signers[1]).withdrawPrincipal(BigNumber.from(10000).mul(multiplier));
  ...
  it("should burn 5,000.00000256 fERC20 for totalYield / 2", async function () {
    ...
    // @ts-ignore
    const args = (receipt.events?.filter((x) => {return x.event == "BurnedFToken"}))[0]['args'];
    // @ts-ignore
  ...
  it("ensure quoting 100,000 DAI for 1 days = 50000.000025600000000000 fDAI", async function () {
    ...
    expect(await this.flashStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).eq("50000000025600000000000")
  });
  ...
  it("ensure quoting for 59 seconds reverts with error DURATION TOO LOW", async function () {
    ...
    const _tokenAmount = BigNumber.from(1).mul(multiplier);
    const _duration = 59;
    await expect(this.flashStrategyAAVEv2Artifact.quoteMintFToken(_tokenAmount, _duration)).to.be.revertedWith("DURATION TOO LOW")
  });
  ...
  it("any ERC20 tokens in the contract can be withdrawn by owner except the interest bearing token (a token)", async function () {
    ...
    await erc20TokenContract.connect(signer).transfer(this.flashStrategyAAVEv2Artifact.address, BigNumber.from(100).mul(multiplier));
    expect(await erc20TokenContract.balanceOf(this.flashStrategyAAVEv2Artifact.address)).to.be.eq(BigNumber.from(100).mul(multiplier));
    ...
    await expect(this.flashStrategyAAVEv2Artifact.connect(this.signers[2]).withdrawERC20(tokensToWithdraw, amountsToWithdraw)).to.be.revertedWith("Ownable: caller is not the owner");

    // Attempt to withdraw as owner (should be fine)
    expect(await this.flashStrategyAAVEv2Artifact.connect(this.signers[0]).withdrawERC20(tokensToWithdraw, amountsToWithdraw)).to.be.ok;
    expect(await erc20TokenContract.balanceOf(this.flashStrategyAAVEv2Artifact.address)).to.be.eq(BigNumber.from(0).mul(multiplier));
    ...
    await aDaiContract.connect(this.signers[0]).transfer(this.flashStrategyAAVEv2Artifact.address, amountsToWithdraw2[0]);
    expect(await aDaiContract.balanceOf(this.flashStrategyAAVEv2Artifact.address)).to.be.eq(amountsToWithdraw2[0]);

    await expect(this.flashStrategyAAVEv2Artifact.connect(this.signers[0]).withdrawERC20(tokensToWithdraw2, amountsToWithdraw)).to.be.revertedWith("TOKEN ADDRESS PROHIBITED");
  });
  ...
  it("should revert with ARRAY SIZE MISMATCH when providing invalid input to withdrawERC20", async function () {
    ...
    await expect(this.flashStrategyAAVEv2Artifact.withdrawERC20(tokensToWithdraw, amountsToWithdraw)).to.be.revertedWith("ARRAY SIZE MISMATCH");
  });

  it("should deposit 10,000 DAI as bootstrap balance", async function () {
    const daiContract = await hre.ethers.getContractAt("IERC20C", principalTokenAddress);
    await daiContract.connect(this.signers[0]).transfer(this.flashStrategyAAVEv2Artifact.address, BigNumber.from(10000).mul(multiplier));
    expect(await daiContract.balanceOf(this.flashStrategyAAVEv2Artifact.address)).to.be.eq(BigNumber.from(10000).mul(multiplier));
  });
});
...
describe("FlashStrategyAAVEv2 Bootstrap Tests", function () {
  ...
  it("[account 0] should deposit 100 DAI as bootstrap balance", async function () {
    const daiContract = await hre.ethers.getContractAt("IERC20C", principalTokenAddress);
    await daiContract.connect(this.signers[0]).transfer(this.flashStrategyAAVEv2Artifact.address, BigNumber.from(100).mul(multiplier));
    expect(await daiContract.balanceOf(this.flashStrategyAAVEv2Artifact.address)).to.be.eq(BigNumber.from(100).mul(multiplier));
  });
  ...
  it("(1) should burn half fDAI for half of available yield paid from only bootstrap balance", async function () {
  ...
  it("(2) should burn remaining fDAI for half of available yield paid from both bootstrap balance and strategy", async function () {
  ...
  it("should ensure quote is not available when fERC20 token supply is 0", async function () {
    await expect(this.flashStrategyAAVEv2Artifact.quoteBurnFToken(BigNumber.from(1).mul(multiplier))).to.be.revertedWith("INSUFFICIENT fERC20 TOKEN SUPPLY")
  });
});
