...
describe.only("FlashBack Tests (same token)", function () {
  ...
  before(async function () {
    ...
    flashBackContract = <FlashBack>(
      await deployContract(signers[0], flashBackContractA, [
        flashTokenContract.address,
        flashTokenContract.address,
        864000,
        31536000,
      ])
  ...
  it("account 1 should fail getting quote, 1,000,000 FLASH for 366 days with error DURATION > MAXIMUM IS 365 DAYS", async function () {
    let _amount = ethers.utils.parseUnits("1000000", 18);
    let _duration = 31622400;

    await expect(flashBackContract.calculateReward(_amount, _duration)).to.be.revertedWith("DURATION > MAXIMUM");
  });

  it("account 1 should fail getting quote, 1,000,000 FLASH for < 10 days with error DURATION < MINIMUM", async function () {
    let _amount = ethers.utils.parseUnits("1000000", 18);
    let _duration = 863999;

    await expect(flashBackContract.calculateReward(_amount, _duration)).to.be.revertedWith("DURATION < MINIMUM");
  });
  ...
  it("account 1 should fail unstaking with error MINIMUM STAKE DURATION IS 10 DAYS", async function () {
    await expect(flashBackContract.connect(signers[1]).unstake(1)).to.be.revertedWith("DURATION < MINIMUM");
  });
...
describe.only("FlashBack Tests (different reward token)", function () {
  ...
  before(async function () {
    ...
    tokenB = <FlashToken>await deployContract(signers[0], flashTokenContractA);
  ...
  it("(4) account 2 should unstake, principal and reward to wallet", async function () {
    const stakeInfo = await flashBackContract.stakes(2);
    const oldPrincipalBalance = await flashTokenContract.balanceOf(signers[2].address);
    ...
    expect(await flashTokenContract.balanceOf(signers[2].address)).to.be.eq(
      oldPrincipalBalance.add(stakeInfo.stakedAmount),
    );
    expect(await tokenB.balanceOf(signers[2].address)).to.be.eq(oldRewardBalance.add(stakeInfo.reservedReward));

    console.log("(4) newPrincipalBalance", await flashTokenContract.balanceOf(signers[2].address));
